  ...
  answerCard: (quality) => {
    ...
    try {
      ...
      if (targetDeck) {
        ...
        // Update the card in the deck store (async, but don't await for immediate UI response)
        deckStore.updateCard(targetDeck.id, currentCard.id, {
          ...
          // Trigger deck statistics recalculation after successful save
          setTimeout(() => {
        ...
        console.log('✅ Card update initiated');
